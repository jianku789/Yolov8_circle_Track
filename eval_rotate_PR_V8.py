...
def creat_annotions(label_path):
    ...
    for label_name in label_files:
        ...
        # print("Labels array l:", l)
        # print("Length of l:", nl)
        # print("labelname:",label_name)
        ...
        for cls_i in range(len(cls_name_list)):     
            for j in range(nl):
                ...
                else:print(f"Attempted to access index {j} which is out of bounds for array of size {len(l)}")
...
if __name__ == '__main__':
    ...
    weights='runs/train/exp80/weights/best.pt'
    img_path='datasets/circle_0412_zuizhong/images/val'
    label_path='datasets/circle_0412_zuizhong/labelTxt/val'
    cls_name_list=['circle']
    out_path = "run_eval"
    iou_thr = 0.5
    ...
    for img_name in img_files:
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            pred = model(im)
            # NMS
            pred = non_max_suppression_obb(pred, 0.5, 0.25, multi_label=True, max_det=1000)
    ...
    mean_ap_robbox, eval_rbbox_results =  eval_rbbox_map(det_list,annotions)

    # Plot and save PR curve
    for cls_index, cls_eval in enumerate(eval_rbbox_results):
        recalls = cls_eval['recall']
        precisions = cls_eval['precision']

        plt.figure()
        plt.plot(recalls, precisions, label=f'Class {cls_index}')
        plt.xlabel('Recall')
        plt.ylabel('Precision')
        plt.title(f'PR Curve for Class {cls_index} at IoU {iou_thr}')
        plt.legend()
        plt.grid(True)

        # 设置Precision轴的刻度间隔
        plt.yticks(np.arange(0, 1.1, step=0.1))  # 从0到1，间隔为0.1

        plt.savefig(osp.join(out_path, f'PR_Curve_Class_{cls_index}_IoU_{int(iou_thr * 100):02d}.png'))
        plt.close()
