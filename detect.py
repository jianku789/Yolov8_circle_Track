...
from utils.rboxs_utils import dets2rbox, poly2rbox, rbox2poly
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                olddet = det
                olddet[:, :4] = scale_coords(im.shape[2:], det[:, :4], im0.shape)
                # det[:, :4] = scale_coords(im.shape[2:], det[:, :4], im0.shape).round()
                roboxs = olddet[:, :4]
                # rboxs= dets2rbox(det)
                pred_poly = scale_polys(im.shape[2:], pred_poly, im0.shape)
                print('pred_poly',pred_poly)
                det = torch.cat((pred_poly,roboxs,det[:, -3:]), dim=1) # (n, [poly rbox θ conf cls])
                ...
                # Write results
                count = 0
                for *poly_robox,θ, conf, cls in reversed(det):
                    poly= poly_robox[:8]
                    robox = poly_robox[8:12]
                    print(θ)
                    # print('poly:',*poly)
                    if save_txt:  # Write to file
                        # xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                        # poly = poly.tolist()
                        line = (count, *robox, conf) if save_conf else (count, *robox)  # label format
                    ...
                    if save_img or save_crop or view_img:  # Add poly to image
                        ...
                        annotator.ellipse_label(robox,θ)
