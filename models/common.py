...
#----------------------------ESAM边缘增强-----------------
def get_sobel_filters(in_chan, out_chan, device, dtype=torch.float32):
    # 创建Sobel滤波器
    sobel_x = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=dtype).reshape(1, 1, 3, 3)
    sobel_y = torch.tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]], dtype=dtype).reshape(1, 1, 3, 3)
    # 扩展滤波器维度以匹配输入和输出通道
    sobel_x = sobel_x.repeat(out_chan, in_chan, 1, 1).to(device)
    sobel_y = sobel_y.repeat(out_chan, in_chan, 1, 1).to(device)
    return sobel_x, sobel_y

def apply_sobel(input, sobel_x, sobel_y):
    g_x = F.conv2d(input, sobel_x, padding=1)
    g_y = F.conv2d(input, sobel_y, padding=1)
    g = torch.sqrt(g_x ** 2 + g_y ** 2)
    return torch.sigmoid(g) * input

class ESAM(nn.Module):
    def __init__(self, in_chan,out_chan, device=0):
        super(ESAM, self).__init__()
        # 先初始化所有带参数的层
        self.conv1 = nn.Conv2d(in_chan, in_chan, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv2d(in_chan, in_chan, kernel_size=1)
        self.bn = nn.BatchNorm2d(in_chan)

        # 确保有参数后再尝试获取参数数据类型
        sample_param = next(self.parameters(), None)  # 获取一个参数样本，如果没有则返回 None
        self.dtype = sample_param.dtype if sample_param is not None else torch.float32
        # 获取 Sobel 滤波器
        self.sobel_x, self.sobel_y = get_sobel_filters(in_chan, in_chan, device, self.dtype)

    def forward(self, x):
        y = apply_sobel(x, self.sobel_x, self.sobel_y)
        y = F.relu(self.bn(y))
        y = self.conv1(y)
        y = x + y
        y = self.conv2(y)
