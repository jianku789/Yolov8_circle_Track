...
class Annotator:
    ...
    def poly_label(self, poly, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        # else:
            ...
            if label:
                ...
                x_label, y_label = int(xmax), int(ymax)
    ...
    # 画椭圆
    def ellipse_label(self, robox, theta, color=(0, 200, 0), thickness=2):
        # Convert tensor to numpy if necessary and extract robox components.
        if isinstance(robox[0], torch.Tensor):
            cx, cy, width, height = [x.item() for x in robox]
        else:
            cx, cy, width, height = robox
        ...
        # Ensure the longer side of the rectangle is the major axis of the ellipse.
        # The rotation might swap what is considered width and height.
        if abs(theta_degrees) > 45:
            # When the angle suggests the major axis is what we'd normally call 'height',
            # swap the dimensions.
            box_size = (int(height/2), int(width/2))
        else:
            box_size = (int(width/2), int(height/2))

        # Center coordinates need to be tuples of integers.
        center_coordinates = (int(cx), int(cy))

        # Adjust theta for OpenCV's drawing function. 
        # OpenCV defines the angle in the clockwise direction, so no adjustment is necessary for theta_degrees.
        
        # Draw the ellipse on the image.
        cv2.ellipse(self.im_cv2, center_coordinates, box_size, theta_degrees, 0, 360, color, thickness)
